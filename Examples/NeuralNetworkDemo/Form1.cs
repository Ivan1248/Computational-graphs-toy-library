...
namespace NeuralNetworksTest
{
    public partial class Form1 : Form
    {
        ...
        private GraphicalLearningMachine ann = CreateNet2();
        ...
        private static GraphicalLearningMachine CreateNet2()
        {
            InputNode inputNode = Nodes.Input(1), labelNode = Nodes.Input(1);
            var h = Nodes.Input(1);
            h.Set(Vector.Of(0.5));
            var annOutputNode = inputNode.WeightedSums(9).Add().Tanh().WeightedSums(1).Add();
            var lossNode = annOutputNode.SquaredLoss(labelNode);
            var trainer = new Trainer(
                inputNode: inputNode,
                labelNode: labelNode,
                lossNode: lossNode,
                optimizer: new MomentumRMSPropOptimizer(0.0006, gamma: 0.9, momentum: 0.99),
                nodes: annOutputNode.CollectPreceedingNodes(filter: n => n.Parameters.Length > 0),
                X: new List<Vector>(), Y: new List<Vector>(),
                batchSize: 1
            );
            return new GraphicalLearningMachine(trainer);
        }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            plot.Series.Add(new FunctionSeries(Predict, 0, 10, 0.1, "ann(x)"));
            RefreshPlotAndInfo();
            plot.MouseDown += (s, ev) =>
            {
        ...
        private double Predict(double x) => ann.Predict(Vector.Filled(1, x))[0];
        ...
        {
            this.x.Add(Vector.Of(x));
            this.y.Add(Vector.Of(y));
        ...
        private void RemovePoint(double x, double y, double tol = 5)
        {
            ...
            {
                if (Vector.Of((x - this.x[i][0]) / dx, (y - this.y[i][0]) / dy).Norm() > 10) continue;
                this.x.RemoveAt(i);
                this.y.RemoveAt(i);
